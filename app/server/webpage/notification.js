...
function setup_website() {
    ...
    function setHeaders (){
        const metaData = {
        title: "InsightBridge: Semantic Q&A",
        description: "Ask questions on your document corpus (Preprints of cs.CL June '25) using a FAISS index and HuggingFace LLM.",
        image: "https://llm.koenix.de/pwa/desktop_icons/icon_512.png",
        author: "Ferdinand Koenig",
        type: "website", // Open Graph type
        url: "https://llm.koenix.de",
        site_name: "InsightBridge",
        locale: "en_US"
        };
        ...
        function replaceMeta(attr, value, propertyType = "property") {
            document.querySelectorAll(`meta[${propertyType}="${attr}"]`).forEach(m => m.remove());
            const m = document.createElement('meta');
            m.setAttribute(propertyType, attr);
            m.setAttribute('content', value);
            document.head.appendChild(m);
        }

        // Open Graph / LinkedIn
        replaceMeta("og:type", metaData.type);
        replaceMeta("og:title", metaData.title);
        replaceMeta("og:description", metaData.description);
        replaceMeta("og:image", metaData.image);
        replaceMeta("og:url", metaData.url);
        replaceMeta("og:site_name", metaData.site_name);
        replaceMeta("og:locale", metaData.locale);

        // Twitter
        replaceMeta("twitter:card", "summary_large_image", "name");
        replaceMeta("twitter:title", metaData.title, "name");
        replaceMeta("twitter:description", metaData.description, "name");
        replaceMeta("twitter:image", metaData.image, "name");
        replaceMeta("twitter:domain", "llm.koenix.de", "property");
        replaceMeta("twitter:url", metaData.url, "property");

        // Standard meta tags
        replaceMeta("description", metaData.description, "name");
        replaceMeta("author", metaData.author, "name");

        document.querySelectorAll(`meta[name="twitter:creator"]`).forEach(m => m.remove());
    }
